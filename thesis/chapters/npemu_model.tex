...
such as Haensel and Pichon~\cite{haensel_pichon1994}, Friedman and
...
ground-state nucleus corresponds to a first-order phase transition at fixed
...
$\rho=\rho_{\mathrm{cc}}$.  
The resulting combined EoS is thermodynamically consistent, numerically
stable, and suitable for the stellar structure calculations presented in the
...
\subsection{Comparison with observationally inferred EoS and mass--radius bands}
...
pressure--density band. The accompanying \texttt{ns} group contains the
corresponding neutron-star sequences—mass, radius, and tidal deformability
computed by Ng~\textit{et al.}—as well as the observationally inferred
mass--radius posterior samples themselves. Both the modelled sequences and
the observational samples are used to form the mass--radius band shown here.

In all figures we restrict the axes to the same density, pressure, mass, and
radius ranges displayed in Fig.~4 of Ng~\textit{et al.}. These limits are
taken to define the domain over which the PSR+GW+X-ray posterior is intended
to be interpreted. We also limit the datasets to the 90\% credible range like
in the original publication.

Figure~\ref{fig:eos_band} shows the pressure--density relation of the
$npe\mu$ core EoS together with the PSR+GW+X-ray band from
Ng~\textit{et al.}. Only the uniform-matter part of the model is used here.
For context, the crust--core transition of the crust models we have used lie around
$\log_{10}\rho \approx 14$, which is near the lower end of the density range
covered by the Ng~\textit{et al.} band.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.70\textwidth]{figures/npemu/eos_band}
...
This indicates that the RMF model we have used might
omit softening mechanisms that would reduce the
pressure near and somewhat above the crust--core transition density.
There is also a very slight deviation around $\log_10\rho~15$, where the model
is below the band.
Nevertheless, the core EoS remains mostly within the 90\% credible region.

\smallskip

For the mass--radius comparison we use the full stellar model, which includes
the FPS crust at low densities. The resulting comparison is displayed
in Fig.~\ref{fig:mr_band}.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/npemu/mr_band}
  \caption[Mass--radius curve compared with observational posterior band]{
    Mass--radius relation for the FPS+$npe\mu$ model compared with the
    90\% PSR+GW+X-ray posterior band of Ng \textit{et al.}.
    Stable configurations are shown with solid lines, unstable ones dashed.}
  \label{fig:mr_band}
\end{figure}

The mass--radius curve shows a broadly consistent trend when compared with the
observed band, though the agreement is not uniform across the entire mass
range. At high masses, around $M\simeq 2\,M_{\odot}$, the predicted radius lies
close to the upper edge of the credible region. As the mass decreases, the model
curve moves to systematically larger radii and gradually departs from the
posterior band. This behaviour reflects the slight over-stiffness already seen
in the pressure comparison: a stiffer EoS yields larger radii at fixed mass.

Toward lower masses the radius grows more rapidly. In this regime the stellar
structure becomes increasingly dominated by the outer layers, and differences in
the low-density part of the EoS---including the details of the crust---leave a
clear imprint. This part of the sequence lies outside the range that is tightly
constrained in the Ng \textit{et al.} analysis.

Taken together, the comparisons in Figs.~\ref{fig:eos_band} and
\ref{fig:mr_band} show that the FPS+$npe\mu$ model captures the overall
behaviour supported by current constraints, though with a
tendency toward slightly larger radii at intermediate masses. These deviations
point to the limitations of the minimal nucleonic RMF model
and motivate the exploration of more flexible or extended models.
