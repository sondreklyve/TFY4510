...
are plotted on a logarithmic scale as functions of the mass density $\rho$.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/npemu/comp}
...
rest mass and the muon density is numerically zero in the solution.
...
chemical potential $\mu_{e}$ eventually reaches the muon rest mass, at a
density that can be read off from the point where the muon curve in
Fig.~\ref{fig:npemu_composition} first lifts off the numerical zero level.
...
\label{subsec:core_eos_results}

With the composition determined at each density, the next step is to
construct the corresponding equation of state $P(\epsilon)$ for
uniform $npe\mu$ matter. The numerical implementation solves the
mean–field equations and equilibrium conditions together with the
thermodynamic expressions of Sec.~\ref{subsec:rmf_selfconsistent}.
For each mass density $\rho$ on the numerical grid, the code returns the
converged fields, particle densities, energy density, and pressure, thereby
producing the full core EoS.

A direct way to assess the plausibility of the obtained EoS is to compare
it with observationally constrained pressure–density bands.
Figure~\ref{fig:npemu_eos_vs_ng} shows the resulting $npe\mu$ core EoS
together with the 95\% credible pressure band from the recent
semiparametric analysis of Ng \textit{et al.}~\cite{Ng_2025}.
The observational band synthesizes information from massive pulsars,
binary-merger tidal deformabilities, and NICER X-ray pulse-profile
measurements, and therefore represents one of the most up-to-date and
model-independent constraints currently available.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.70\textwidth]{figures/npemu/eos_bands}
  \caption[Core EoS compared with observational band]{
    Pressure--density relation of the RMF $npe\mu$ core EoS compared
    with the 95\% credible region from Ng \textit{et al.}
    (PSR + GW + NICER).
    The $npe\mu$ model lies comfortably within the observationally
    allowed region across the entire density range shown. A slight
    offset appears in the intermediate region but the curve otherwise
    remains fully inside the 95\% credible band.
  }
  \label{fig:npemu_eos_vs_ng}
\end{figure}

The comparison shows that the RMF-based $npe\mu$ core EoS is broadly
consistent with the observational band across the entire density interval
displayed. At the lower and higher ends of the range the model follows the
posterior band closely, and the overall slope agrees well with the
observational trend. In the intermediate region around
$\log_{10}\rho \sim 14.2$--$14.5$ the EoS lies slightly above the central
part of the band, producing a modest systematic offset. This indicates that the
interaction parameters calibrated to nuclear bulk properties produce an
EoS that is largely compatible with astrophysical constraints without any further
adjustment.

A modest offset of this kind is not unexpected given the minimal nature of
the $npe\mu$ framework. Although the RMF description is entirely applicable
in this uniform-matter regime, it represents only the leading contribution
to the microphysics: the model includes nucleons and leptons but omits
additional mechanisms that are known to soften the EoS near and moderately
above saturation density. Examples include many-body correlations beyond the
mean field or possible additional degrees of freedom. Such effects tend to
lower the pressure in precisely the density range where the slight
displacement appears. Within this context it is natural that the $npe\mu$
EoS sits toward the upper part of the allowed region, while still remaining
fully consistent with the observational band.

It is also important to emphasise that all densities shown in
Fig.~\ref{fig:npemu_eos_vs_ng} lie well within the uniform-matter domain
where the RMF $npe\mu$ model is physically valid. The comparison therefore
tests the core EoS only, not the low-density crustal regime. To obtain a
complete neutron-star EoS suitable for TOV integration down to vanishing
pressure, the core description must still be supplemented by an appropriate
model for the outer layers. This motivates the introduction of a dedicated
crust EoS in the next subsection.
...
requirement that the polytrope matches smoothly onto the RMF $npe\mu$ core
...
fix the constant $K$ so that the polytrope connects smoothly to the
$npe\mu$ core EoS.
\begin{equation}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/npemu/poly_eos}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/npemu/poly_mr}
...
of the maximum mass, and the onset of instability, determined by the $npe\mu$ core description.
...
To obtain a physically motivated description of the outer layers of a neutron
star, the microphysics of the crust must be taken into account. The essential
features are well established and follow the classic work of Baym, Pethick
and Sutherland (BPS)~\cite{bps1971}, and later refinements such as the
Friedman–Pandharipande–Skyrme (FPS)~\cite{fps1981} and
Skyrme–Lyon (SLy)~\cite{sly2001} models. A clear summary is given in
Pogliano’s thesis (see Chapter~5, pp.~69–72), where the structure of crustal
matter and the corresponding EoS is presented in detail. The results used
here follow the FPS analytical approximation~\cite{pogliano2017}, which
provides a smooth and accurate parametrization suitable for TOV integration.

At very low densities the matter consists of a Coulomb lattice of iron-like
nuclei embedded in a degenerate electron gas. As the density increases,
heavier and more neutron-rich nuclei become energetically favoured. This
sequence of composition changes appears as a series of phase transitions,
each occurring at fixed pressure and slightly different nuclear species. When
the density reaches the neutron-drip threshold—visible as the break near
$4 \times 10^{11}\,\mathrm{g/cm^{3}}$ in the tabulated models—the most weakly bound
neutrons escape the nuclei and populate a free neutron gas between the
lattice sites. Above this point, in the inner crust, the nuclei continue to
grow in size and neutron excess while the free neutron component increases.
Near the crust–core interface the lattice becomes unstable against
non-spherical nuclear shapes (“pasta phases”), but the overall effect on the
pressure is modest and well captured by parametrized EoS models such as FPS.

The key point for the present work is that the crust is intrinsically
non-uniform and cannot be described by the RMF $npe\mu$ model. The field
equations developed earlier assume a uniform medium of interacting baryons,
which is appropriate only above the crust–core transition density
($\rho \gtrsim 10^{14}\,\mathrm{g/cm^{3}}$). Below this threshold, the physics is
dominated by nuclear structure and lattice geometry. Consequently, 
the core EoS must be complemented by a crust model
that captures these physical regimes and connects smoothly down to the
surface.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\textwidth]{figures/npemu/crust_eos}
  \caption[Crust + core equation of state]{
    ...
    phases and joins smoothly onto the uniform core matter. For comparison,
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/npemu/crust_mr}
...
sub-nuclear densities. This combined EoS is consistent with current
astrophysical constraints, produces realistic radii across the mass range,
and removes the arbitrariness present in the naive polytropic extension.
