...
where support against gravity is provided by electron degeneracy
...
radial dependence of pressure $P(r)$ and enclosed mass $m(r)$ in a
...
The missing input required to close the system is an EOS $P(\epsilon)$.
...
\epsilon_0 = m_n^4,
\]
where $G$ is Newton's constant and $m_n$ the neutron mass. All
quantities are then expressed in units of $(r_0, m_0, \epsilon_0)$,
so that
\[
\hat r = r/r_0, \qquad
\hat m = m/m_0, \qquad
...
\begin{enumerate}
  \item The TOV equations with boundary conditions at the center and
  surface.
...
Herman Sletmoen as part of his Master's thesis~\cite{sletmoen2022}.
...
one can extract the global properties of the star, namely its mass $M$
and radius $R$, and thereby construct the mass--radius relation.
In addition, the program provides access to internal structure such as
the radial pressure profile $P(r)$ and energy density $\epsilon(r)$,
which offer further insight into how equilibrium is maintained inside
the star. In the following section we present both global and internal
results for ideal neutron stars.
...
Figure~\ref{fig:mr_neutron} shows the mass--radius curves obtained under
